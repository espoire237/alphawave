...
const ServiceCard = ({ svc, revealed, learnMore }) => {
  ...
  return (
    <div data-reveal onMouseEnter={()=>setHovered(true)} onMouseLeave={()=>setHovered(false)}
      ...
      <Link to={`/services#${svc.slug}`} style={{ textDecoration:"none", display:"inline-flex", alignItems:"center", gap:5, fontFamily:FONTS.primary, fontSize:FONTS.size.xs, fontWeight:FONTS.weight.bold, color:hovered?MY_COLORS.orange:MY_COLORS.textDisabled, transition:"color 0.3s ease" }}>
...
const HomeServices = () => {
  ...
  // ✅ SERVICES inside component so t() is in scope
  const SERVICES = [
    { id:1, icon:"code",    title:t("homeServices.services.software.title"),  desc:t("homeServices.services.software.desc"),  slug:"custom-software"        },
    { id:2, icon:"brain",   title:t("homeServices.services.ai.title"),        desc:t("homeServices.services.ai.desc"),        slug:"ai-analytics"           },
    { id:3, icon:"chart",   title:t("homeServices.services.marketing.title"), desc:t("homeServices.services.marketing.desc"), slug:"digital-marketing"      },
    { id:4, icon:"cloud",   title:t("homeServices.services.cloud.title"),     desc:t("homeServices.services.cloud.desc"),     slug:"cloud-devops"           },
    { id:5, icon:"mobile",  title:t("homeServices.services.mobile.title"),    desc:t("homeServices.services.mobile.desc"),    slug:"mobile-first"           },
    { id:6, icon:"payment", title:t("homeServices.services.payment.title"),   desc:t("homeServices.services.payment.desc"),   slug:"payment-infrastructure" },
  ];

  useEffect(() => {
    const el = sectionRef.current; if (!el) return;
    const obs = new IntersectionObserver(([e]) => { if (e.isIntersecting) { setRevealed(true); obs.disconnect(); } }, { threshold:0.06, rootMargin:"0px 0px -40px 0px" });
        ...
        <div style={{ display:"flex", alignItems:isMobile?"flex-start":"flex-end", justifyContent:"space-between", flexDirection:isMobile?"column":"row", gap:16, marginBottom:isMobile?32:56 }}>
          <div>
            <div data-reveal style={{ display:"inline-flex", alignItems:"center", gap:10, marginBottom:14, opacity:revealed?1:0, transform:revealed?"translateY(0)":"translateY(20px)", transition:"opacity 0.6s ease, transform 0.6s ease" }}>
              ...
              <span style={{ fontFamily:FONTS.primary, fontSize:FONTS.size.xs, fontWeight:FONTS.weight.bold, letterSpacing:FONTS.tracking.widest, textTransform:"uppercase", color:MY_COLORS.orange }}>{t("homeServices.eyebrow")}</span>
              <span style={{ width:28, height:2, borderRadius:9999, background:MY_COLORS.gradientOrange }}/>
            </div>
            <h2 data-reveal style={{ fontFamily:FONTS.primary, fontSize:isMobile?"clamp(22px,6vw,32px)":"clamp(28px,4vw,48px)", fontWeight:FONTS.weight.extrabold, color:MY_COLORS.textPrimary, margin:0, opacity:revealed?1:0, transform:revealed?"translateY(0)":"translateY(20px)", transition:"opacity 0.6s ease 0.1s, transform 0.6s ease 0.1s" }}>
              {t("homeServices.title")} <span style={{ color:MY_COLORS.orange }}>{t("homeServices.titleAccent")}</span>
            </h2>
          </div>
          <Link data-reveal to="/services" style={{ textDecoration:"none", display:"inline-flex", alignItems:"center", gap:6, fontFamily:FONTS.primary, fontSize:FONTS.size.sm, fontWeight:FONTS.weight.bold, color:MY_COLORS.orange, opacity:revealed?1:0, transform:revealed?"translateY(0)":"translateY(20px)", transition:"opacity 0.6s ease 0.2s, transform 0.6s ease 0.2s", whiteSpace:"nowrap", flexShrink:0 }}>
            {t("homeServices.viewAll")} <ArrowRight/>
          </Link>
        </div>
        <div style={{ display:"grid", gridTemplateColumns:gridCols, gap:isMobile?12:20 }}>
          {SERVICES.map(svc => <ServiceCard key={svc.id} svc={svc} revealed={revealed} learnMore={t("homeServices.learnMore")}/>)}
        </div>
      </div>
    </section>
  );
