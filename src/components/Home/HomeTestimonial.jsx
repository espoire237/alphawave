...
const TESTIMONIAL = {
  quote:   "AlphaWaves transformed our retail business completely. The mobile money integration alone increased our online sales by 250% in just three months. Their team understood our market challenges and delivered a solution that actually works.",
...
const StarIcon = () => (
  <svg width="16" height="16" viewBox="0 0 16 16" fill={MY_COLORS.orange}>
    ...
    <section ref={sectionRef} style={{ position: "relative", background: MY_COLORS.bgSection, padding, overflow: "hidden" }}>
      ...
      <div style={{ maxWidth: isMobile ? "100%" : 860, margin: "0 auto", position: "relative", textAlign: "center" }}>
        ...
        <div data-reveal style={{ display: "inline-flex", alignItems: "center", gap: 10, marginBottom: isMobile ? 24 : 36, opacity: 0, transform: "translateY(20px)", transition: "all 0.6s ease" }}>
          ...
          <span style={{ fontFamily: FONTS.primary, fontSize: FONTS.size.xs, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: MY_COLORS.orange }}>Client Story</span>
          <span style={{ width: 28, height: 2, borderRadius: 9999, background: MY_COLORS.gradientOrange }} />
        </div>

        {/* Giant quote mark — hidden on smallest mobile */}
        {!isMobile && (
          <div data-reveal style={{ fontFamily: "Georgia, serif", fontSize: 100, lineHeight: 0.6, color: MY_COLORS.orangeBorder, marginBottom: 24, userSelect: "none", opacity: 0, transform: "translateY(20px)", transition: "all 0.6s ease" }}>"</div>
        )}
        ...
        <div data-reveal style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: isMobile ? 12 : 16, flexWrap: "wrap", opacity: 0, transform: "translateY(20px)", transition: "all 0.6s ease" }}>
          ...
          <div style={{ textAlign: "left" }}>
            <div style={{ fontFamily: FONTS.primary, fontSize: FONTS.size.base, fontWeight: 700, color: MY_COLORS.textPrimary }}>{TESTIMONIAL.author}</div>
            <div style={{ fontFamily: FONTS.secondary, fontSize: FONTS.size.xs, color: MY_COLORS.textMuted }}>{TESTIMONIAL.title} · {TESTIMONIAL.company}</div>
          </div>
