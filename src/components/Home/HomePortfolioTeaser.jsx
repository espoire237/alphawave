...
const FEATURED_BASE = [
  { id:1, slug:"ecommerce-fashion-retailer", key:"ecommerce",   metric:"250%", accent:MY_COLORS.orange },
  { id:2, slug:"fintech-lending-platform",   key:"fintech",     metric:"70%",  accent:"#60a5fa"        },
  { id:3, slug:"agricultural-marketplace",   key:"agriculture", metric:"500+", accent:"#4ade80"        },
];
...
const ProjectCard = ({ project, viewCase }) => {
  ...
  return (
    <div data-reveal onMouseEnter={()=>setHovered(true)} onMouseLeave={()=>setHovered(false)}
      ...
      <div style={{ height:180, background:`linear-gradient(135deg, ${project.accent}22 0%, rgba(10,10,10,0.95) 100%)`, position:"relative", display:"flex", alignItems:"center", justifyContent:"center", overflow:"hidden" }}>
        <div style={{ fontFamily:FONTS.primary, fontSize:72, fontWeight:800, color:`${project.accent}12`, lineHeight:1, userSelect:"none" }}>{project.title.split(" ").map(w=>w[0]).join("").slice(0,2)}</div>
        <div style={{ position:"absolute", top:12, left:12, padding:"4px 12px", borderRadius:9999, background:"rgba(10,10,10,0.75)", border:`1px solid ${project.accent}44`, fontFamily:FONTS.primary, fontSize:10, fontWeight:700, letterSpacing:"0.08em", textTransform:"uppercase", color:project.accent, backdropFilter:"blur(8px)" }}>{project.category}</div>
      ...
      <div style={{ padding:"20px" }}>
        ...
        <div style={{ display:"flex", gap:6, marginBottom:14, flexWrap:"wrap" }}>
          {project.tech.map((t2,i) => <span key={i} style={{ padding:"3px 10px", borderRadius:9999, background:"rgba(255,255,255,0.04)", border:`1px solid ${MY_COLORS.border}`, fontFamily:FONTS.secondary, fontSize:10, color:MY_COLORS.textMuted }}>{t2}</span>)}
        </div>
...
const HomePortfolioTeaser = () => {
  ...
  // ✅ FEATURED with translated fields inside component
  const FEATURED = FEATURED_BASE.map(p => ({
    ...p,
    category:    t(`homePortfolio.projects.${p.key}.category`),
    title:       t(`homePortfolio.projects.${p.key}.title`),
    metricLabel: t(`homePortfolio.projects.${p.key}.metricLabel`),
    tech: p.key === "ecommerce" ? ["React.js","MTN MoMo"] : p.key === "fintech" ? ["Python","AI/ML"] : ["React Native","Maps"],
  }));
        ...
        <div style={{ display:"flex", alignItems:isMobile?"flex-start":"flex-end", justifyContent:"space-between", flexDirection:isMobile?"column":"row", gap:16, marginBottom:isMobile?28:56 }}>
          <div>
            <div data-reveal style={{ display:"inline-flex", alignItems:"center", gap:10, marginBottom:14, opacity:0, transform:"translateY(20px)", transition:"all 0.6s ease" }}>
              ...
              <span style={{ fontFamily:FONTS.primary, fontSize:FONTS.size.xs, fontWeight:700, letterSpacing:"0.1em", textTransform:"uppercase", color:MY_COLORS.orange }}>{t("homePortfolio.eyebrow")}</span>
              <span style={{ width:28, height:2, borderRadius:9999, background:MY_COLORS.gradientOrange }}/>
            </div>
            <h2 data-reveal style={{ fontFamily:FONTS.primary, fontSize:isMobile?"clamp(22px,6vw,32px)":"clamp(28px,4vw,48px)", fontWeight:800, color:MY_COLORS.textPrimary, margin:0, opacity:0, transform:"translateY(20px)", transition:"all 0.6s ease" }}>
              {t("homePortfolio.title")} <span style={{ color:MY_COLORS.orange }}>{t("homePortfolio.titleAccent")}</span>
            </h2>
          </div>
          <Link to="/portfolio" data-reveal onMouseEnter={()=>setCtaH(true)} onMouseLeave={()=>setCtaH(false)}
            ...
            {t("homePortfolio.viewAll")} <ArrowRight/>
          </Link>
        </div>
        <div style={{ display:"grid", gridTemplateColumns:gridCols, gap:isMobile?14:24 }}>
          {(isMobile ? FEATURED.slice(0,2) : FEATURED).map(p => <ProjectCard key={p.id} project={p} viewCase={t("homePortfolio.viewCase")}/>)}
        </div>
        {isMobile && (
          <div style={{ textAlign:"center", marginTop:20 }}>
            <Link to="/portfolio" style={{ textDecoration:"none", display:"inline-flex", alignItems:"center", gap:6, padding:"11px 24px", borderRadius:10, border:`1px solid ${MY_COLORS.border}`, color:MY_COLORS.textMuted, fontFamily:FONTS.primary, fontSize:FONTS.size.sm, fontWeight:700, transition:"all 0.3s ease" }}
              ...
              {t("homePortfolio.seeAll")} <ArrowRight/>
            </Link>
          </div>
        )}
      </div>
