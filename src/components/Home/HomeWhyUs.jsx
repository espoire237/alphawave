...
const POINT_ICONS = [
  <svg width="28" height="28" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="9" stroke="currentColor" strokeWidth="1.6"/><path d="M12 3C12 3 8 7 8 12C8 17 12 21 12 21M12 3C12 3 16 7 16 12C16 17 12 21 12 21M3 12H21" stroke="currentColor" strokeWidth="1.6"/></svg>,
  <svg width="28" height="28" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="2" stroke="currentColor" strokeWidth="1.6"/><circle cx="4" cy="6" r="2" stroke="currentColor" strokeWidth="1.6"/><circle cx="20" cy="6" r="2" stroke="currentColor" strokeWidth="1.6"/><circle cx="4" cy="18" r="2" stroke="currentColor" strokeWidth="1.6"/><circle cx="20" cy="18" r="2" stroke="currentColor" strokeWidth="1.6"/><path d="M6 7L10 11M14 13L18 17M18 7L14 11M10 13L6 17" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round"/></svg>,
  <svg width="28" height="28" viewBox="0 0 24 24" fill="none"><path d="M3 20H21M5 20V14M9 20V8M13 20V11M17 20V4" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round"/></svg>,
];
const POINT_NUMBERS = ["01","02","03"];
const POINT_KEYS    = ["1","2","3"];
...
const HomeWhyUs = () => {
  ...
  // ✅ POINTS inside component so t() is in scope
  const POINTS = POINT_KEYS.map((key, i) => ({
    id:     i+1,
    number: POINT_NUMBERS[i],
    title:  t(`homeWhyUs.points.${key}.title`),
    desc:   t(`homeWhyUs.points.${key}.desc`),
    icon:   POINT_ICONS[i],
  }));
          ...
          <div>
            <div data-reveal style={{ opacity:0, transform:"translateY(20px)", transition:"all 0.6s ease" }}>
              <div style={{ display:"inline-flex", alignItems:"center", gap:10, marginBottom:20 }}>
                ...
                <span style={{ fontFamily:FONTS.primary, fontSize:FONTS.size.xs, fontWeight:FONTS.weight.bold, letterSpacing:FONTS.tracking.widest, textTransform:"uppercase", color:MY_COLORS.orange }}>{t("homeWhyUs.eyebrow")}</span>
              </div>
              <h2 style={{ fontFamily:FONTS.primary, fontSize:isMobile?"clamp(22px,6vw,32px)":"clamp(28px,3.5vw,44px)", fontWeight:FONTS.weight.extrabold, color:MY_COLORS.textPrimary, margin:"0 0 16px", lineHeight:"1.15" }}>
                {t("homeWhyUs.title")} <span style={{ color:MY_COLORS.orange }}>{t("homeWhyUs.titleAccent")}</span>
              </h2>
              <p style={{ fontFamily:FONTS.secondary, fontSize:FONTS.size.base, lineHeight:FONTS.leading.relaxed, color:MY_COLORS.textMuted, margin:"0 0 28px" }}>{t("homeWhyUs.subtitle")}</p>
              {!isSmall && (
                <Link to="/about" style={{ textDecoration:"none", display:"inline-flex", alignItems:"center", gap:8, padding:"11px 22px", borderRadius:10, border:`1px solid ${MY_COLORS.orangeBorder}`, color:MY_COLORS.orange, fontFamily:FONTS.primary, fontSize:FONTS.size.sm, fontWeight:FONTS.weight.bold, transition:"all 0.3s ease" }}
                  ...
                  {t("homeWhyUs.aboutBtn")} <ArrowRight/>
                </Link>
              )}
            </div>
          </div>
          <div style={{ display:"flex", flexDirection:"column", gap:isMobile?12:16 }}>
            {POINTS.map(point => (
              <div key={point.id} data-reveal style={{ display:"flex", gap:isMobile?16:24, padding:isMobile?"20px 16px":"28px", borderRadius:14, background:MY_COLORS.bgSurface, border:`1px solid ${MY_COLORS.border}`, opacity:0, transform:"translateY(24px)", transition:"all 0.6s ease", position:"relative", overflow:"hidden" }}>
                ...
                <div style={{ flexShrink:0, width:isMobile?44:52, height:isMobile?44:52, borderRadius:12, background:MY_COLORS.orangeDim, border:`1px solid ${MY_COLORS.orangeBorder}`, display:"flex", alignItems:"center", justifyContent:"center", color:MY_COLORS.orange }}>{point.icon}</div>
                <div>
