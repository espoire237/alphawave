...
>
      ...
      {CITIES.map((city, i) => (
        <g key={`n-${i}`} filter="url(#storyNodeGlow)">
          ...
          >
            <animate
              attributeName="r"
              values={city.hub ? "2.5;7;2.5" : "1.8;5;1.8"}
              dur={`${2.4 + i * 0.45}s`}
              repeatCount="indefinite"
            />
            <animate
    ...
    <div
      style={{
        ...
        borderRadius: "50%",
        background:
          "radial-gradient(ellipse, rgba(232,117,10,0.07) 0%, transparent 70%)",
        pointerEvents: "none",
      }}
    />
  </div>
...
const OurStory = () => {
  ...
  const statValueSize = isMobile
    ? "clamp(20px,5vw,26px)"
    : "clamp(22px,2.5vw,30px)";
  const statPad = isMobile ? "14px 12px" : "20px 16px";
  const paraInitTransform = isSingleCol
    ? "translateY(24px)"
    : "translateX(30px)";
    ...
    >
      ...
      >
        ...
        >
          {/* ══ LEFT COLUMN ══ */}
          <div
            style={{ position: isSingleCol ? "static" : "sticky", top: 120 }}
          >
            <div
              data-reveal
              style={{
            ...
            >
              From Cameroon to the world — building technology that works in
              Africa and competes globally.
            </p>
            ...
            {PARAGRAPHS.map((para, index) => (
              <div
                ...
                style={{
                  position: "relative",
                  paddingBottom:
                    index < PARAGRAPHS.length - 1 ? (isMobile ? 32 : 48) : 0,
                  opacity: 0,
                  transform: paraInitTransform,
                  transition: "opacity 0.6s ease, transform 0.6s ease",
                }}
              >
