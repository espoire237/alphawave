 ...
 * Featuring: Spotlight Hover Effects & Africa-Ready Stats
 */
...
const SpotlightCard = ({ col, idx }) => {
  ...
  return (
    <div
      ...
      style={{ 
        background: col.highlight ? `linear-gradient(145deg, rgba(232,117,10,0.1) 0%, rgba(232,117,10,0.04) 100%)` : MY_COLORS.bgSurface,
        borderColor: col.highlight ? MY_COLORS.orangeBorder : MY_COLORS.border,
    ...
    >
      ...
      <div 
        ...
        style={{
          ...
          background: `radial-gradient(600px circle at ${mousePos.x}px ${mousePos.y}px, rgba(232,117,10,0.08), transparent 40%)`
        }}
      />
      ...
      <div className="relative z-10">
        <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-7 border transition-all duration-300 group-hover:scale-110 group-hover:shadow-[0_0_20px_rgba(232,117,10,0.3)]"
          style={{ 
            background: col.highlight ? MY_COLORS.orangeDim : MY_COLORS.bgSurfaceHover,
            borderColor: col.highlight ? MY_COLORS.orangeBorder : MY_COLORS.border,
        ...
        <div className="h-0.5 rounded-full mb-5 transition-all duration-300 w-8 group-hover:w-12"
          style={{ background: MY_COLORS.gradientOrange }} />
        ...
        <div className="flex items-center gap-4 pt-6 border-t" style={{ borderColor: MY_COLORS.border }}>
          <span className="text-3xl font-black" style={{ fontFamily: FONTS.primary, color: MY_COLORS.orange }}>
            {col.stat}
          </span>
          <span className="text-xs uppercase font-bold tracking-wider leading-tight" style={{ color: MY_COLORS.textMuted }}>
            {col.statLabel}
          </span>
        </div>
      </div>
...
const WhyCameroon = () => {
  ...
  const COLUMNS = [
    { id: 1, icon: <GlobeIcon />, headline: "Bilingual by Nature", content: "Operating in both French and English gives us unique access to Francophone West/Central Africa and Anglophone markets — covering 500+ million people.", stat: "500M+", statLabel: "Potential Reach", highlight: true },
    { id: 2, icon: <NetworkIcon />, headline: "Regional Hub", content: "Cameroon's strategic position connects us to major African markets: Nigeria, Gabon, Chad, CAR, and the broader CEMAC/ECOWAS regions.", stat: "10+", statLabel: "Neighboring Markets", highlight: false },
    { id: 3, icon: <BuildingIcon />, headline: "Real-World Testing", content: "Building here means our solutions are tested against real infrastructure challenges — ensuring they work everywhere on the continent.", stat: "100%", statLabel: "Africa-Ready", highlight: false },
  ];
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-10">
        ...
        <div className="text-center mb-16 md:mb-20">
          ...
          <h2 data-reveal className="text-3xl md:text-5xl lg:text-6xl font-extrabold tracking-tight mb-6 opacity-0 translate-y-5 transition-all duration-700 delay-100 ease-out"
            ...
            Why <span style={{ color: MY_COLORS.orange }}>Cameroon?</span>
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16 md:mb-20">
          {COLUMNS.map((col, idx) => (
            <SpotlightCard key={col.id} col={col} idx={idx} />
          ))}
        </div>
        ...
        <div data-reveal className="relative p-8 md:p-14 rounded-3xl border flex flex-col lg:flex-row items-center justify-between gap-10 overflow-hidden opacity-0 translate-y-10 transition-all duration-700 delay-500"
          ...
          <div className="relative z-10 flex-1 text-center lg:text-left">
            ...
            <h3 className="text-2xl md:text-3xl font-extrabold mb-4 leading-tight" style={{ fontFamily: FONTS.primary, color: MY_COLORS.textPrimary }}>
              Two languages. One continent. <br className="hidden md:block" />
            ...
            <p className="max-w-xl text-base md:text-lg leading-relaxed" style={{ fontFamily: FONTS.secondary, color: MY_COLORS.textSecondary }}>
              Our English and French fluency opens doors across the entire African continent.
            </p>
          </div>

          <div className="flex flex-col gap-4 w-full md:w-auto relative z-10">
            {[{ lang: "English", sub: "Anglophone Africa", flag: "🇬🇧" }, { lang: "Français", sub: "Afrique Francophone", flag: "🇫🇷" }].map((item) => (
              <div key={item.lang} className="flex items-center gap-4 p-4 rounded-xl border bg-black/20 backdrop-blur-sm min-w-[240px] transition-transform hover:scale-105"
                style={{ borderColor: MY_COLORS.border }}>
                <span className="text-2xl">{item.flag}</span>
                <div className="flex-1">
                  <div className="font-bold text-white" style={{ fontFamily: FONTS.primary }}>{item.lang}</div>
                  <div className="text-[10px] uppercase tracking-wider text-gray-400">{item.sub}</div>
                </div>
...
const NetworkIcon = () => (
  <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14"/><path d="M12 5v14"/><circle cx="12" cy="12" r="3"/></svg>
);
const BuildingIcon = () => (
  <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="4" y="2" width="16" height="20" rx="2" ry="2"/><line x1="9" y1="22" x2="9" y2="2"/><line x1="15" y1="22" x2="15" y2="2"/><line x1="4" y1="9" x2="20" y2="9"/><line x1="4" y1="15" x2="20" y2="15"/></svg>
);
