...
const PortfolioTestimonials = () => {
  const { t, i18n } = useTranslation(); // 1. hook first
  const lang = i18n.language?.startsWith("fr") ? "fr" : "en"; // 2. lang second
  const testimonials = PROJECTS.filter(
    // 3. testimonials third
    (p) => p.testimonial && p.status === "published",
  ).map((p) => {
    const base = p.testimonial;
    const locale = base[lang] || {};
    return {
      rating: base.rating,
      author: base.author,
      company: base.company,
      project: p.project_name,
      quote: locale.quote || base.quote,
        ...
        >
          ...
          >
            <button
              ...
              onMouseEnter={(e) => {
                e.currentTarget.style.borderColor = MY_COLORS.orange;
            ...
            {testimonials.map((_, i) => (
              <button
                ...
                style={{
                  ...
                  borderRadius: 9999,
                  background:
                    i === active ? MY_COLORS.orange : MY_COLORS.border,
                  border: "none",
                  cursor: "pointer",
