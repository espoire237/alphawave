/**
 * PortfolioCTA — Portfolio Page Section 7 — FULLY RESPONSIVE
 */
...
const TRUST_POINTS = ["Free consultation and project assessment", "Transparent pricing with no hidden costs", "Proven track record across 8+ industries", "Dedicated team committed to your success"];
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", position: "relative" }}>
        <div data-reveal style={{ padding: boxPad, borderRadius: isMobile ? 16 : 24, background: "linear-gradient(135deg, rgba(232,117,10,0.1) 0%, rgba(232,117,10,0.05) 40%, rgba(10,10,10,0.9) 100%)", border: `1px solid ${MY_COLORS.orangeBorder}`, overflow: "hidden", textAlign: "center", position: "relative", opacity: 0, transform: "translateY(30px) scale(0.98)", transition: "opacity 0.7s ease, transform 0.7s ease" }}>
          ...
          <div data-reveal style={{ display: "inline-flex", alignItems: "center", gap: 10, marginBottom: isMobile ? 14 : 20, opacity: 0, transform: "translateY(16px)", transition: "all 0.6s ease" }}>
            ...
            <span style={{ fontFamily: FONTS.primary, fontSize: FONTS.size.xs, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: MY_COLORS.orange }}>Start Your Project</span>
            <span style={{ width: 24, height: 2, borderRadius: 9999, background: MY_COLORS.gradientOrange }} />
          </div>
          <h2 data-reveal style={{ fontFamily: FONTS.primary, fontSize: isMobile ? "clamp(22px,6vw,32px)" : "clamp(28px,4vw,52px)", fontWeight: 800, color: MY_COLORS.textPrimary, margin: `0 auto ${isMobile ? 12 : 16}px`, maxWidth: 640, lineHeight: "1.15", opacity: 0, transform: "translateY(20px)", transition: "all 0.6s ease" }}>
            Ready to Become Our Next <span style={{ color: MY_COLORS.orange }}>Success Story?</span>
          </h2>
          <p data-reveal style={{ fontFamily: FONTS.secondary, fontSize: isMobile ? FONTS.size.sm : FONTS.size.md, lineHeight: "1.7", color: MY_COLORS.textSecondary, margin: `0 auto ${isMobile ? 20 : 36}px`, maxWidth: 520, opacity: 0, transform: "translateY(20px)", transition: "all 0.6s ease" }}>
            Join leading businesses across Africa who have transformed their operations with AlphaWaves.
          </p>
          <div data-reveal style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "auto auto auto auto", justifyContent: isMobile ? "stretch" : "center", gap: isMobile ? "8px 12px" : "10px 24px", marginBottom: isMobile ? 24 : 36, opacity: 0, transform: "translateY(20px)", transition: "all 0.6s ease" }}>
            {TRUST_POINTS.map((point, i) => (
              <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 7 }}>
                <div style={{ flexShrink: 0, marginTop: 2 }}><CheckIcon /></div>
                <span style={{ fontFamily: FONTS.secondary, fontSize: isMobile ? 11 : FONTS.size.sm, color: MY_COLORS.textSecondary, lineHeight: "1.4", textAlign: "left" }}>{point}</span>
              </div>
            ))}
          </div>
