...
const HowWeWork = () => {
  ...
  return (
    <section ref={sectionRef} style={{ position: "relative", background: MY_COLORS.bgBase, padding: "100px 0", overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", bottom: -200, left: -100, width: 600, height: 600, borderRadius: "50%", background: `radial-gradient(circle, ${MY_COLORS.orangeSection} 0%, transparent 65%)`, pointerEvents: "none" }} />
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 40px", position: "relative" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 72 }}>
          <div data-reveal style={{ display: "inline-flex", alignItems: "center", gap: 10, marginBottom: 16, opacity: 0, transform: "translateY(20px)", transition: "all 0.6s ease" }}>
            ...
            <span style={{ fontFamily: FONTS.primary, fontSize: FONTS.size.xs, fontWeight: FONTS.weight.bold, letterSpacing: FONTS.tracking.widest, textTransform: "uppercase", color: MY_COLORS.orange }}>Our Process</span>
            <span style={{ width: 28, height: 2, borderRadius: 9999, background: MY_COLORS.gradientOrange }} />
          </div>
          ...
          <h2 data-reveal style={{ fontFamily: FONTS.primary, fontSize: "clamp(28px, 4vw, 48px)", fontWeight: FONTS.weight.extrabold, letterSpacing: FONTS.tracking.tight, color: MY_COLORS.textPrimary, margin: "0 auto 16px", maxWidth: 600, opacity: 0, transform: "translateY(20px)", transition: "all 0.6s ease" }}>
            From Vision to{" "}
            <span style={{ color: MY_COLORS.orange, textShadow: `0 0 30px ${MY_COLORS.orangeGlow}` }}>Victory</span>
          </h2>
        ...
        <div style={{ position: "relative" }}>
          ...
          {/* Connecting line */}
          <div style={{ position: "absolute", top: 36, left: "10%", right: "10%", height: 1, background: `linear-gradient(to right, transparent, ${MY_COLORS.orangeBorder}, transparent)`, zIndex: 0 }} />
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 16, position: "relative", zIndex: 1 }}>
            {PROCESS_STEPS.map((step, i) => (
              <div
                key={step.id}
                data-reveal
                style={{ display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", opacity: 0, transform: "translateY(24px)", transition: "all 0.6s ease" }}
              >
                {/* Circle */}
                <div style={{
                  width:        72,
                  ...
                  <span style={{ fontFamily: FONTS.primary, fontSize: 10, fontWeight: FONTS.weight.bold, letterSpacing: FONTS.tracking.widest, color: MY_COLORS.orange }}>
                    {step.number}
                  </span>
                </div>

                {/* Title */}
                <h3 style={{ fontFamily: FONTS.primary, fontSize: FONTS.size.sm, fontWeight: FONTS.weight.bold, letterSpacing: FONTS.tracking.tight, color: MY_COLORS.textPrimary, margin: "0 0 10px", lineHeight: FONTS.leading.snug }}>
                  {step.title}
                </h3>

                {/* Desc */}
                <p style={{ fontFamily: FONTS.secondary, fontSize: FONTS.size.xs, lineHeight: FONTS.leading.relaxed, color: MY_COLORS.textMuted, margin: 0 }}>
                  {step.desc}
                </p>
              </div>
            ))}
